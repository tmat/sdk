...
internal sealed class ProcessState(Process process, ILogger logger, ProcessExitAction? onExit, Func<ValueTask<bool>>? terminator, bool isUserApplication) : IDisposable
{
    ...
    /// <summary>
    /// Waits for the process to exit. Always executes onExit handler, if specified, and completes the returned task.
    /// </summary>
    /// <param name="processCleanupTimeout">Time span to give the process for graceful termination. The process will be forcefully killed afterwards.</param>
    /// <param name="processTerminationToken">Cancellation token that triggers process termination.</param>
    /// <returns>Process exit code, or null if it can't be retrieved.</returns>
